...
export default function ChangeItemMenu(props) {
  const { selectedValue, menuName, menuItems, onClick, itemOnClick } = props;
      ...
      {Object.keys(menuItems).map((menuItem) => (
        <Grid
          ...
          sx={{
            cursor: "pointer",
            backgroundColor:
              selectedValue === menuItems[menuItem].label
                ? "CRESTDarkAlt.main"
                : "CRESTGridBackground.dark",
            "&:hover": {
              backgroundColor: "#6f6f6f",
            },
          }}
          onClick={(event) => {
            minimizeOnClick(event);
            itemClick(menuItems[menuItem].label);
          }}
        >
